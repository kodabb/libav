...
static int tak_read_header(AVFormatContext *s)
{
    ...
    while (!pb->eof_reached) {
        ...
        case TAK_METADATA_ENCODER:
            ...
            av_bitstream_get_init(&gb, buffer, size * 8);
            break;
        ...
        if (type == TAK_METADATA_STREAMINFO) {
            ...
            if (size != 11)
                return AVERROR_INVALIDDATA;
            tc->mlast_frame = 1;
            tc->data_end    = av_bitstream_get64(&gb, TAK_LAST_FRAME_POS_BITS) +
                              av_bitstream_get(&gb, TAK_LAST_FRAME_SIZE_BITS);
            av_freep(&buffer);
        } else if (type == TAK_METADATA_ENCODER) {
            av_log(s, AV_LOG_VERBOSE, "encoder version: %0X\n",
                   av_bitstream_get_long(&gb, TAK_ENCODER_VERSION_BITS));
            av_freep(&buffer);
        }
    }
