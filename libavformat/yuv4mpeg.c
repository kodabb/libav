...
static int yuv4_generate_header(AVFormatContext *s, char* buf)
{
    ...
    if (st->codec->coded_frame &&
        (ff_avframe_fieldstate_get(st->codec->coded_frame) & AV_FRAME_INTERLACED))
        inter = ff_avframe_fieldstate_get(st->codec->coded_frame) == AV_FRAME_INTERLACED_TFF ? 't' : 'b';
...
static int yuv4_read_header(AVFormatContext *s)
{
    ...
    AVStream *st;
    enum AVFieldState *s1 = s->priv_data;
        ...
        case 'I': // Interlace type
            ...
            case 'p':
                *s1 = AV_FRAME_PROGRESSIVE;
                break;
            case 't':
                *s1 = AV_FRAME_INTERLACED_TFF;
                break;
            case 'b':
                *s1 = AV_FRAME_INTERLACED_BFF;
                break;
            case 'm':
...
static int yuv4_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (st->codec->coded_frame)
        ff_avframe_fieldstate_set(st->codec->coded_frame, *s1);
...
AVInputFormat ff_yuv4mpegpipe_demuxer = {
    .name           = "yuv4mpegpipe",
    .long_name      = NULL_IF_CONFIG_SMALL("YUV4MPEG pipe"),
    .priv_data_size = sizeof(enum AVFieldState),
