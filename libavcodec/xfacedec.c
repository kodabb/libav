...
static int pop_integer(BigInt *b, const ProbRange *pranges)
{
    ...
    ff_bigint_mul(b, pranges->range);
    ff_bigint_add(b, r - pranges->offset);
    return i;
}
...
{
    ff_set_dimensions(avctx, XFACE_WIDTH, XFACE_HEIGHT);
    avctx->pix_fmt = AV_PIX_FMT_MONOWHITE;

    return 0;
}

static int xface_decode_frame(AVCodecContext *avctx, void *data,
                              int *got_frame, AVPacket *avpkt)
{
    ...
    for (i = 0, k = 0; i < avpkt->size && avpkt->data[i]; i++) {
        ...
        ff_bigint_mul(&b, XFACE_PRINTS);
        ff_bigint_add(&b, c - XFACE_FIRST_PRINT);
    }
