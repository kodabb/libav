                               ...
                               const int data_size, AVCodecContext *avctx)
{
    ...
    if (ctx->frame_type)        /* if interlaced */
        if (ctx->frame_type & 1)
            ff_avframe_fieldstate_set(ctx->frame, AV_FRAME_INTERLACED_TFF);
        else
            ff_avframe_fieldstate_set(ctx->frame, AV_FRAME_INTERLACED_BFF);
    else
        ff_avframe_fieldstate_set(ctx->frame, AV_FRAME_PROGRESSIVE);
                                 ...
                                 const int data_size, AVCodecContext *avctx)
{
    ...
    interlaced = ff_avframe_fieldstate_get(ctx->frame) & AV_FRAME_INTERLACED;
...
static int decode_slice(AVCodecContext *avctx, void *tdata)
{
    ...
    if (ff_avframe_fieldstate_get(pic) & AV_FRAME_INTERLACED) {
        if (!(pic_num ^ (ff_avframe_fieldstate_get(pic) == AV_FRAME_INTERLACED_TFF))) {
            y_data += y_linesize;
            u_data += u_linesize;
            v_data += v_linesize;
            if (a_data)
