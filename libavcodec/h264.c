...
#if CONFIG_ERROR_RESILIENCE
static void h264_copy_to_er(ERPicture *dst, H264Picture *src)
{
    int i;
...
static int field_end(H264Context *h, int in_setup)
{
    ...
    if (CONFIG_ERROR_RESILIENCE && !FIELD_PICTURE(h)) {
        h264_copy_to_er(&h->er.cur_pic, h->cur_pic_ptr);
        h264_copy_to_er(&h->er.last_pic, h->ref_count[0] ? &h->ref_list[0][0] : NULL);
        h264_copy_to_er(&h->er.next_pic, h->ref_count[1] ? &h->ref_list[1][0] : NULL);
