...
static void decode_postinit(H264Context *h, int setup_finished)
{
    ...
    h->cur_pic_ptr->tf.f->pict_type = h->pict_type;
    ...
    cur->tf.f->interlaced_frame = 0;
    cur->tf.f->repeat_pict      = 0;
    ...
    if (h->sps.pic_struct_present_flag) {
        ...
        case SEI_PIC_STRUCT_BOTTOM_TOP:
            if (FIELD_OR_MBAFF_PICTURE(h))
                ...
                cur->tf.f->interlaced_frame = h->prev_interlaced_frame;
            break;
        ...
        case SEI_PIC_STRUCT_BOTTOM_TOP_BOTTOM:
            ...
            cur->tf.f->repeat_pict = 1;
            break;
        case SEI_PIC_STRUCT_FRAME_DOUBLING:
            cur->tf.f->repeat_pict = 2;
            break;
        case SEI_PIC_STRUCT_FRAME_TRIPLING:
            cur->tf.f->repeat_pict = 4;
            break;
        }

        if ((h->sei_ct_type & 3) &&
            h->sei_pic_struct <= SEI_PIC_STRUCT_BOTTOM_TOP)
            cur->tf.f->interlaced_frame = (h->sei_ct_type & (1 << 1)) != 0;
    } else {
        /* Derive interlacing flag from used decoding process. */
        cur->tf.f->interlaced_frame = FIELD_OR_MBAFF_PICTURE(h);
    }
    h->prev_interlaced_frame = cur->tf.f->interlaced_frame;

    if (cur->field_poc[0] != cur->field_poc[1]) {
        /* Derive top_field_first from field pocs. */
        cur->tf.f->top_field_first = cur->field_poc[0] < cur->field_poc[1];
    } else {
        if (cur->tf.f->interlaced_frame || h->sps.pic_struct_present_flag) {
            /* Use picture timing SEI information. Even if it is a
             * information of a past frame, better than nothing. */
            if (h->sei_pic_struct == SEI_PIC_STRUCT_TOP_BOTTOM ||
                h->sei_pic_struct == SEI_PIC_STRUCT_TOP_BOTTOM_TOP)
                cur->tf.f->top_field_first = 1;
            else
                cur->tf.f->top_field_first = 0;
        } else {
    ...
    if (h->sei_frame_packing_present &&
        ...
        AVStereo3D *stereo = av_stereo3d_create_side_data(cur->tf.f);
        if (!stereo)
            return;
    ...
    if (h->sei_display_orientation_present &&
        ...
        AVFrameSideData *rotation = av_frame_new_side_data(cur->tf.f,
    ...
    if (!h->mmco_reset && !cur->tf.f->key_frame &&
    ...
    for (i = 1; i < MAX_DELAYED_PIC_COUNT &&
                ...
                !h->delayed_pic[i]->tf.f->key_frame;
         i++)
    ...
    if (h->avctx->has_b_frames == 0 &&
        (h->delayed_pic[0]->tf.f->key_frame || h->mmco_reset))
        h->next_outputed_poc = INT_MIN;
    out_of_order = !out->tf.f->key_frame && !h->mmco_reset &&
    ...
    if (h->sps.bitstream_restriction_flag &&
        ...
        h->low_delay = 0;
               ...
               ((h->next_outputed_poc != INT_MIN &&
                ...
                cur->tf.f->pict_type == AV_PICTURE_TYPE_B)) {
        h->low_delay = 0;
    ...
    if (!out_of_order && pics > h->avctx->has_b_frames) {
        ...
        if (out->mmco_reset) {
            ...
            if (out_idx == 0 && pics > 1 && h->delayed_pic[0]->tf.f->key_frame) {
                h->next_outputed_poc = INT_MIN;
            } else {
                h->next_outputed_poc = out->poc;
            }
...
out:
    if (buf_size == 0) {
        ...
        if (out) {
            ret = output_frame(h, pict, out->tf.f);
            if (ret < 0)
                return ret;
            *got_frame = 1;
        }
    ...
    if (!(avctx->flags2 & CODEC_FLAG2_CHUNKS) ||
        ...
        *got_frame = 0;
        if (h->next_output_pic && ((avctx->flags & CODEC_FLAG_OUTPUT_CORRUPT) ||
            ...
            if (!h->next_output_pic->recovered)
                h->next_output_pic->tf.f->flags |= AV_FRAME_FLAG_CORRUPT;

            ret = output_frame(h, pict, h->next_output_pic->tf.f);
            if (ret < 0)
                return ret;
            *got_frame = 1;
        }
