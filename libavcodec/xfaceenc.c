...
static void encode_block(char *bitmap, int w, int h,
                         int level, ProbRangesQueue *pq)
{
...
static void push_integer(BigInt *b, const ProbRange *prange)
{
    ...
    ff_bigint_div(b, prange->range, &r);
    ff_bigint_mul(b, 0);
    ff_bigint_add(b, r + prange->offset);
}
                              ...
                              const AVFrame *frame, int *got_packet)
{
    ...
    while (b.nb_words) {
        uint8_t r;
        ff_bigint_div(&b, XFACE_PRINTS, &r);
