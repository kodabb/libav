...
s->pps->min_tb_addr_zs[(y) * s->sps->pic_width_in_min_tbs + (x)]
    ...
    int minBlockAddrCurr =
            MIN_TB_ADDR_ZS((xCurr >> s->sps->log2_min_transform_block_size), (yCurr >> s->sps->log2_min_transform_block_size));

    int minBlockAddrN;

    if ((xN < 0) || (yN < 0) || (xN >= s->sps->pic_width_in_luma_samples)
            || (yN >= s->sps->pic_height_in_luma_samples)) {
        minBlockAddrN = -1;
    } else {
        minBlockAddrN =
                MIN_TB_ADDR_ZS((xN >> s->sps->log2_min_transform_block_size), (yN >> s->sps->log2_min_transform_block_size));
    }
...
static int compareMVrefidx(struct MvField A, struct MvField B)
{
    ...
    else if (A.pred_flag[0] && !A.pred_flag[1] && B.pred_flag[0]
            && !B.pred_flag[1])
        return ((A.ref_idx[0] == B.ref_idx[0]) && (A.mv[0].x == B.mv[0].x)
                && (A.mv[0].y == B.mv[0].y));
    else if (!A.pred_flag[0] && A.pred_flag[1] && !B.pred_flag[0]
                                         ...
                                         RefPicList* refPicList_col)
{
    ...
    if (temp_col.is_intra) {
        ...
        if (temp_col.pred_flag[0] == 0) {
            ...
            listCol = L1;
            check_mvset = 1;
        } else if ((temp_col.pred_flag[0] == 1)
                && (temp_col.pred_flag[1] == 0)) {
            mvCol = temp_col.mv[0];
            refidxCol = temp_col.ref_idx[0];
            listCol = L0;
            check_mvset = 1;
        } else if ((temp_col.pred_flag[0] == 1)
                && (temp_col.pred_flag[1] == 1)) {
        ...
        if (check_mvset == 1) {
            int currIsLongTerm = refPicList[X].isLongTerm[refIdxLx];
            int colIsLongTerm = refPicList_col[listCol].isLongTerm[refidxCol];
                                            ...
                                            struct MvField mergecandlist[])
{
    ...
    if (s->sh.slice_type == B_SLICE) {
        if ((numOrigMergeCand > 1)
            ...
            combStop = 0;
            while (combStop != 1) {
                struct MvField l0Cand;
                struct MvField l1Cand;
    ...
    if (s->sh.slice_type == P_SLICE) {
        ...
        numRefIdx =
                s->sh.num_ref_idx_l0_active > s->sh.num_ref_idx_l1_active ?
                        s->sh.num_ref_idx_l1_active :
                        s->sh.num_ref_idx_l0_active;
    }
                              ...
                              int mvp_lx_flag, int LX)
{
    ...
    is_available_a0 = (cand_bottom_left
            && !(TAB_MVF(xA0_pu, yA0_pu).is_intra));
    if (is_available_a0)
