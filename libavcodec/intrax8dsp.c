 ...
 * @param range pointer to the variable where the edge pixel range is
 *              to be stored (max-min values)
 * @param psum pointer to the variable where the edge pixel sum is to be stored
 * @param edges informs this routine that the block is on an image border,
 *              so it has to interpolate the missing edge pixels.
 * @note Some of the edge pixels should be interpolated, the flag has the
 *       following meaning:
 *   1   - mb_x == 0 - first block in the row, interpolate area #1, #2, #3;
 *   2   - mb_y == 0 - first row, interpolate area #3, #4, #5, #6;
 *   1|2 - mb_x == mb_y == 0 - first block, use 0x80 value for all areas;
