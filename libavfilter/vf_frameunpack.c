                                         ...
                                         int view)
{
    ...
    // memcpy is all that is right and good
...
static int request_frame_mono(AVFilterLink *outlink)
{
    ...
    // for temporal packing, process one frame after the other
    // for spacial packing, request a new frame only when two frames are output
    if (s->format == AV_STEREO3D_FRAMESEQUENCE ||
        (!s->left_done && !s->right_done)) {
    ...
    if (s->format == AV_STEREO3D_2D) {
        ...
        int64_t orig_pts = s->frame->pts;
        // passthrough/frame dup
        dst = s->frame;
        s->frame = NULL;
        // adjust pts
        dst->pts = av_rescale_q(orig_pts, ctx->inputs[0]->time_base,
                                outlink->time_base);
    } else {
