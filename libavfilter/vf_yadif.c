...
static int return_frame(AVFilterContext *ctx, int is_second)
{
    ...
    if (yadif->parity == -1) {
        tff = ff_avframe_fieldstate_get(yadif->cur) == AV_FRAME_INTERLACED_TFF;
    } else {
        tff = yadif->parity ^ 1;
    }
    ...
    if (is_second) {
        ...
        ff_avframe_fieldstate_set(yadif->out, AV_FRAME_PROGRESSIVE);
    }
...
static int filter_frame(AVFilterLink *link, AVFrame *frame)
{
    ...
    if (yadif->auto_enable &&
        ff_avframe_fieldstate_get(yadif->cur) == AV_FRAME_PROGRESSIVE) {
    ...
    av_frame_copy_props(yadif->out, yadif->cur);
    ff_avframe_fieldstate_set(yadif->cur, AV_FRAME_PROGRESSIVE);
...
static int poll_frame(AVFilterLink *link)
{
    ...
    if (yadif->auto_enable && yadif->next &&
        ff_avframe_fieldstate_get(yadif->next) == AV_FRAME_PROGRESSIVE)
        return val;
