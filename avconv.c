    ...
    *frame_size = 0;
    ...
    if (s->oformat->flags & AVFMT_RAWPICTURE &&
        ...
        /* raw pictures are written as AVPicture structure to
           ...
           method. */
        ff_avframe_fieldstate_set(enc->coded_frame,
                                  ff_avframe_fieldstate_get(in_picture));
        ...
        if (ost->st->codec->flags & (CODEC_FLAG_INTERLACED_DCT|CODEC_FLAG_INTERLACED_ME) &&
            ...
            if (ost->top_field_first)
                ff_avframe_fieldstate_set(in_picture, AV_FRAME_INTERLACED_TFF);
            else
                ff_avframe_fieldstate_set(in_picture, AV_FRAME_INTERLACED_BFF);
        } else
            ff_avframe_fieldstate_set(in_picture, AV_FRAME_PROGRESSIVE);
